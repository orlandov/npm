...
cache.completion = function(args, index, cb) {
  ...
  if (subcmdList.indexOf(subcmd) !== -1) {
    ...
    } else if (index < 4 && (subcmd === "clean" || subcmd === "ls")) {
      ls_("", npm.cache, function (er, cached) {
        if (er) return cb(er)
        cb(null, getCompletions(args[1] || "", cached))
      })
    }
...
function ls (args, cb) {
  ...
  ls_(show, read, function(er, files) {
